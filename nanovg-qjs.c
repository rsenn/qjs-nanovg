...
static const JSCFunctionListEntry js_nanovg_paint_funcs[] = {
...
static int js_nanovg_init(JSContext *ctx, JSModuleDef *m)
{
    ...
    JS_SetModuleExportList(ctx, m, js_nanovg_paint_funcs, countof(js_nanovg_paint_funcs));
    return 0;
}
...
JSModuleDef *js_init_module_nanovg(JSContext *ctx, const char *module_name)
{
    ...
    JS_AddModuleExportList(ctx, m, js_nanovg_paint_funcs, countof(js_nanovg_paint_funcs));
    return m;
}
